...
// Renders the full kitchen interior (runs inside the 720×64 kitchen zone)

interface StoveProps { chef: Chef; x: number }

function PixelStove({ chef, x }: StoveProps) {
  const cooking   = chef.state === 'COOKING';
  const foodReady = chef.state === 'FOOD_READY';

  return (
    <div style={{ position:'absolute', left:x, top:4, width:60, height:52, imageRendering:'pixelated' }}>
      {/* Stove body */}
...
export function KitchenView({ chefs }: Props) {
  return (
    <div style={{ position:'relative', width:'100%', height:'100%', overflow:'hidden', imageRendering:'pixelated' }}>
      ...
      {/* Pass-through counter bar (very top of kitchen zone) */}
      <div style={{ position:'absolute', top:0, left:0, right:0, height:6,
        background:'#6B7280', borderTop:'2px solid #9CA3AF' }} />

      {/* Main counter surface */}
      <div style={{ position:'absolute', top:6, left:0, right:0, height:10,
        background:'#4B5563', borderBottom:'2px solid #374151' }}>
      ...
      {/* Stoves — one per chef */}
      {chefs.map((chef, i) => (
        <PixelStove key={chef.id} chef={chef} x={16 + i * 72} />
      ))}

      {/* Serving window / pass area (right side) */}
      <div style={{ position:'absolute', right:8, top:4, width:90, height:44,
        background:'#374151', border:'2px solid #4B5563' }}>
        {/* Window opening */}
        <div style={{ position:'absolute', top:4, left:4, right:4, height:28,
        ...
        <div style={{ position:'absolute', bottom:4, left:0, right:0, textAlign:'center',
          fontSize:8, color:'#9CA3AF', fontFamily:'monospace' }}>
          PASS
        </div>
      </div>

      {/* Utensil rack on left wall */}
      <div style={{ position:'absolute', left:4, top:8, width:6, height:48,
        background:'#374151', border:'1px solid #4B5563' }}>
        {[12,22,32,42].map((ty,i)=>(
          <div key={i} style={{ position:'absolute', left:1, top:ty, width:4, height:6,
            background:'#9CA3AF' }} />
        ))}
      </div>
    </div>
